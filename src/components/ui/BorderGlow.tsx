...
import React, { useRef, useCallback, useEffect, type ReactNode, forwardRef } from 'react';
import './BorderGlow.css';

interface BorderGlowProps {
  children?: ReactNode;
  className?: string;
  style?: React.CSSProperties; // Transform ve Z-index gibi değerler için eklendi
...
function parseHSL(hslStr: string) {
  ...
  if (!match) return { h: 220, s: 80, l: 60 }; // Mavi tonlarına uyarlandı
...
// MİMARİ DOKUNUŞ: dnd-kit ile uyumlu çalışması için forwardRef eklendi
const BorderGlow = forwardRef<HTMLDivElement, BorderGlowProps>(({
  children, className = '', style = {}, edgeSensitivity = 30, glowColor = '220 80 60', // Indigo rengine ayarlandı
  backgroundColor = '#ffffff', // Aydınlık tema
  borderRadius = 12, glowRadius = 40, glowIntensity = 1.0, coneSpread = 25, animated = false,
  colors = ['#818cf8', '#38bdf8', '#c084fc'], // Aydınlık temaya uygun indigo/mavi renkler
  fillOpacity = 0.3,
}, externalRef) => {
  const internalRef = useRef<HTMLDivElement>(null);

  // İki ref'i birleştiriyoruz (Dnd-kit ve BorderGlow Matematiği için)
  ...
  return (
    <div
      ...
      style={{
        ...
        ...style // Sürükle-bırak transformları için eklendi
